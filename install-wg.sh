...
# Reference https://medium.com/@oliverswitzer_91379/an-in-depth-guide-how-to-setup-your-helium-hotspot-over-a-cellular-network-39763d443c99
...
display_usage() {
	cat <<EOF
Usage: $0 [WG_SERVER_HOSTNAME] [WG_SERVER_PORT] [IPV4_3_FIELDS] [IPV6_HEXTET] [PORT_FORWARDING_DESTINATIONS] [ADDITIONAL_CLIENTS] [ADDITIONAL_PEERS]
...
echo
echo "######################################"
echo "### Running wg server setup script ###"
echo "######################################"
...
# generate iptables commands for port forwarding
SERVER_POSTUP_CONNTRACK=""
SERVER_POSTUP_DNAT=""
SERVER_POSTDOWN_CONNTRACK=""
SERVER_POSTDOWN_DNAT=""
CLIENT_POSTUPDOWN=""
...
do
	...
	if [ ! -z $FORWARD_PORT ]; then
		SERVER_POSTUP_CONNTRACK+="iptables -A FORWARD -i $INTERNET_INF -o \$1 -p $FORWARD_PROTOCOL --syn --dport $FORWARD_PORT -m conntrack --ctstate NEW -j ACCEPT${NEWLINE}"
		SERVER_POSTUP_DNAT+="iptables -t nat -A PREROUTING -i $INTERNET_INF -p $FORWARD_PROTOCOL --dport $FORWARD_PORT -j DNAT --to-destination $IPV4_3_FIELDS.2${NEWLINE}"
		SERVER_POSTDOWN_CONNTRACK+="iptables -D FORWARD -i $INTERNET_INF -o \$1 -p $FORWARD_PROTOCOL --syn --dport $FORWARD_PORT -m conntrack --ctstate NEW -j ACCEPT${NEWLINE}"
		SERVER_POSTDOWN_DNAT+="iptables -t nat -D PREROUTING -i $INTERNET_INF -p $FORWARD_PROTOCOL --dport $FORWARD_PORT -j DNAT --to-destination $IPV4_3_FIELDS.2${NEWLINE}"
